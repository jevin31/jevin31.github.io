      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="max-w-6xl mx-auto">
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            ...
            <div
              ref={imageRef}
              className={`relative transition-all duration-700 ${imageInView
            ...
            >
              <div className="relative w-64 h-64 sm:w-80 sm:h-80 mx-auto">
                ...
                <div className="relative w-full h-full rounded-2xl bg-card border border-border overflow-hidden flex items-center justify-center">
                  {/* Place an avatar at `public/avatar.jpg`. Falls back to emoji if the image fails to load. */}
                  {/** Use `/avatar.jpg` (copy your image to `public/avatar.jpg`) */}
                  <ImageAvatar />
                </div>
              </div>
            </div>
            ...
            <div
              ref={contentRef}
              className={`text-center lg:text-left transition-all duration-700 delay-150 ${contentInView
                  ? "opacity-100 translate-x-0"
                  : "opacity-0 translate-x-10"
                }`}
            >
              ...
              <div className="space-y-4 text-muted-foreground">
                <p>
                  I'm a final year Computer Science student at the University of Mauritius with a deep passion for building
              ...
              <div className="mt-8 flex flex-wrap justify-center lg:justify-start gap-3">
                {highlights.map(({ icon: Icon, label }, index) => (
                  <div
                    key={label}
                    className={`flex items-center gap-2 px-4 py-2 rounded-lg bg-card border border-border text-sm font-medium text-foreground hover:border-primary/50 transition-all duration-500 ${contentInView
